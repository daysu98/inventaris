@extends('layouts.main')

@section('title', 'Data Barang')

@section('content')
    <!-- ====== Table Section Start -->
    <div class="flex flex-col gap-10">
        @if (session()->has('message'))
            <!-- Alerts Item -->
            <div class=" flex items-center w-full border-l-6 border-[#34D399] bg-[#34D399] bg-opacity-[15%] p-7 shadow-md dark:bg-opacity-30">
                <div class="mr-5 flex h-9 w-full max-w-[36px] items-center justify-center rounded-lg bg-[#34D399]">
                    <svg width="13" height="13" viewBox="0 0 13 13" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path
                            d="M6.4917 7.65579L11.106 12.2645C11.2545 12.4128 11.4715 12.5 11.6738 12.5C11.8762 12.5 12.0931 12.4128 12.2416 12.2645C12.5621 11.9445 12.5623 11.4317 12.2423 11.1114C12.2422 11.1113 12.2422 11.1113 12.2422 11.1113C12.242 11.1111 12.2418 11.1109 12.2416 11.1107L7.64539 6.50351L12.2589 1.91221L12.2595 1.91158C12.5802 1.59132 12.5802 1.07805 12.2595 0.757793C11.9393 0.437994 11.4268 0.437869 11.1064 0.757418C11.1063 0.757543 11.1062 0.757668 11.106 0.757793L6.49234 5.34931L1.89459 0.740581L1.89396 0.739942C1.57364 0.420019 1.0608 0.420019 0.740487 0.739944C0.42005 1.05999 0.419837 1.57279 0.73985 1.89309L6.4917 7.65579ZM6.4917 7.65579L1.89459 12.2639L1.89395 12.2645C1.74546 12.4128 1.52854 12.5 1.32616 12.5C1.12377 12.5 0.906853 12.4128 0.758361 12.2645L1.1117 11.9108L0.758358 12.2645C0.437984 11.9445 0.437708 11.4319 0.757539 11.1116C0.757812 11.1113 0.758086 11.111 0.75836 11.1107L5.33864 6.50287L0.740487 1.89373L6.4917 7.65579Z"
                            fill="#ffffff" stroke="#ffffff"></path>
                    </svg>
                </div>
                <div class="w-full">
                    <h5 class="font-bold text-[#34D399]">
                        {{ session()->get('message') }}
                    </h5>
                </div>
            </div>
            <!-- Alerts Item End -->
        @endif
        <!-- ====== Table Three Start -->
        <div class="rounded-sm border border-stroke bg-white px-5 pb-2.5 pt-6 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
            @if (auth()->user()->role_id == 1 || auth()->user()->role_id == 3)
                <div class="flex flex-wrap gap-5 xl:gap-7.5 mb-4">
                    <a href="{{ route('items.create') }}" class="inline-flex items-center justify-center gap-2.5 rounded-md bg-primary px-5 py-2 text-center font-medium text-white hover:bg-opacity-90">
                        <span>
                            <svg xmlns="http://www.w3.org/2000/svg" class="fill-current " width="20" height="20" fill="none" viewBox="0 0 20 20">
                                <path fill="currentColor" d="M11 9h4v2h-4v4H9v-4H5V9h4V5h2zm-1 11a10 10 0 1 1 0-20a10 10 0 0 1 0 20m0-2a8 8 0 1 0 0-16a8 8 0 0 0 0 16" />
                            </svg>
                        </span>
                        <span class="hidden sm:flex">Tambah Barang</span>
                    </a>
                </div>
            @endif
            <div class="max-w-full overflow-x-auto">
                <table class="w-full table-auto">
                    <thead>
                        <tr class="bg-gray-2 text-left dark:bg-meta-4">
                            <th class="min-w-[50px] px-4 py-4 font-medium text-black dark:text-white xl:pl-11">
                                No
                            </th>
                            <th class="min-w-[250px] px-4 py-4 font-medium text-black dark:text-white">
                                Produk
                            </th>
                            <th class="min-w-[100px] px-4 py-4 font-medium text-black dark:text-white">
                                Stok
                            </th>
                            <th class="min-w-[150px] px-4 py-4 font-medium text-black dark:text-white">
                                Harga
                            </th>
                            <th class="min-w-[150px] px-4 py-4 font-medium text-black dark:text-white">
                                Supplier
                            </th>
                            <th class="min-w-[150px] px-4 py-4 font-medium text-black dark:text-white">
                                Petugas
                            </th>
                            @if (auth()->user()->role_id == 1 || auth()->user()->role_id == 4)
                                <th class="px-4 py-4 font-medium text-black dark:text-white">
                                    Publish
                                </th>
                            @endif
                            @if (auth()->user()->role_id == 1 || auth()->user()->role_id == 2 || auth()->user()->role_id == 4)
                                <th class="px-4 py-4 font-medium text-black dark:text-white">
                                    Opsi
                                </th>
                            @endif
                        </tr>
                    </thead>
                    <tbody>
                        @foreach ($items as $item)
                            <tr>
                                <td class="border-b border-[#eee] px-4 py-5 pl-9 dark:border-strokedark xl:pl-11">
                                    <h5 class="font-medium text-black dark:text-white">{{ $loop->iteration }}</h5>
                                </td>
                                <td class="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                                    <div class="flex flex-col gap-4 sm:flex-row sm:items-center">
                                        <div class="h-15 w-15 rounded-md">
                                            <img src="{{ asset('storage/' . $item->photo) }}" />
                                        </div>
                                        <p class="text-sm font-medium text-black dark:text-white">
                                            {{ $item->name }}
                                        </p>
                                    </div>
                                </td>
                                <td class="border-b border-[#eee] px-4 fpy-5 dark:border-strokedark">
                                    <p class="text-black dark:text-white">{{ $item->stock }}</p>
                                </td>
                                <td class="border-b border-[#eee] px-4 fpy-5 dark:border-strokedark">
                                    <p class="text-black dark:text-white" style="text-wrap: nowrap;">Rp {{ number_format($item->price, 0, ',', '.') }}</p>
                                </td>
                                <td class="border-b border-[#eee] px-4 fpy-5 dark:border-strokedark">
                                    <p class="text-black dark:text-white">{{ $item->supplier->name }}</p>
                                </td>
                                <td class="border-b border-[#eee] px-4 fpy-5 dark:border-strokedark">
                                    <p class="text-black dark:text-white">{{ $item->user->name }}</p>
                                </td>
                                @if (auth()->user()->role_id == 1 || auth()->user()->role_id == 4)
                                    <td class="border-b border-[#eee] px-4 fpy-5 dark:border-strokedark">
                                        <form action="{{ route('items.publish', ['item' => $item]) }}" method="POST">
                                            @csrf
                                            @method('PATCH')
                                            @if ($item->publish == false)
                                                <button type="submit" class="inline-flex items-center justify-center gap-2.5 rounded-full border border-primary px-10 py-4 text-center font-medium text-primary hover:bg-opacity-90 lg:px-8 xl:px-10">
                                                    <span>
                                                        <svg class="h-4 w-4 stroke-current" fill="none" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                                                        </svg>
                                                    </span>
                                                </button>
                                            @else
                                                <button type="submit" class="inline-flex items-center justify-center gap-2.5 rounded-full bg-primary px-10 py-4 text-center font-medium text-white hover:bg-opacity-90 lg:px-8 xl:px-10">
                                                    <span>
                                                        <svg class="fill-current stroke-current" width="11" height="8" viewBox="0 0 11 8" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                            <path
                                                                d="M10.0915 0.951972L10.0867 0.946075L10.0813 0.940568C9.90076 0.753564 9.61034 0.753146 9.42927 0.939309L4.16201 6.22962L1.58507 3.63469C1.40401 3.44841 1.11351 3.44879 0.932892 3.63584C0.755703 3.81933 0.755703 4.10875 0.932892 4.29224L0.932878 4.29225L0.934851 4.29424L3.58046 6.95832C3.73676 7.11955 3.94983 7.2 4.1473 7.2C4.36196 7.2 4.55963 7.11773 4.71406 6.9584L10.0468 1.60234C10.2436 1.4199 10.2421 1.1339 10.0915 0.951972ZM4.2327 6.30081L4.2317 6.2998C4.23206 6.30015 4.23237 6.30049 4.23269 6.30082L4.2327 6.30081Z"
                                                                fill="" stroke="" stroke-width="0.4"></path>
                                                        </svg>
                                                    </span>
                                                </button>
                                            @endif
                                        </form>
                                    </td>
                                @endif
                                @if (auth()->user()->role_id == 1 || auth()->user()->role_id == 2 || auth()->user()->role_id == 4)
                                    <td class="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                                        <div class="flex items-center space-x-3.5">
                                            @if (auth()->user()->role_id == 1 || auth()->user()->role_id == 2)
                                                <a href="{{ route('items.edit', ['item' => $item]) }}" class="hover:text-primary">
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="fill-current" width="18" height="18" viewBox="0 0 24 24">
                                                        <g fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2">
                                                            <path d="M12 3H5a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7" />
                                                            <path d="M18.375 2.625a2.121 2.121 0 1 1 3 3L12 15l-4 1l1-4Z" />
                                                        </g>
                                                    </svg>
                                                </a>
                                            @endif
                                            @if (auth()->user()->role_id == 1 || auth()->user()->role_id == 4)
                                                <form action="{{ route('items.destroy', ['item' => $item->id]) }}" method="POST" style="height: 20px;">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="hover:text-primary">
                                                        <svg class="fill-current" width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                            <path
                                                                d="M13.7535 2.47502H11.5879V1.9969C11.5879 1.15315 10.9129 0.478149 10.0691 0.478149H7.90352C7.05977 0.478149 6.38477 1.15315 6.38477 1.9969V2.47502H4.21914C3.40352 2.47502 2.72852 3.15002 2.72852 3.96565V4.8094C2.72852 5.42815 3.09414 5.9344 3.62852 6.1594L4.07852 15.4688C4.13477 16.6219 5.09102 17.5219 6.24414 17.5219H11.7004C12.8535 17.5219 13.8098 16.6219 13.866 15.4688L14.3441 6.13127C14.8785 5.90627 15.2441 5.3719 15.2441 4.78127V3.93752C15.2441 3.15002 14.5691 2.47502 13.7535 2.47502ZM7.67852 1.9969C7.67852 1.85627 7.79102 1.74377 7.93164 1.74377H10.0973C10.2379 1.74377 10.3504 1.85627 10.3504 1.9969V2.47502H7.70664V1.9969H7.67852ZM4.02227 3.96565C4.02227 3.85315 4.10664 3.74065 4.24727 3.74065H13.7535C13.866 3.74065 13.9785 3.82502 13.9785 3.96565V4.8094C13.9785 4.9219 13.8941 5.0344 13.7535 5.0344H4.24727C4.13477 5.0344 4.02227 4.95002 4.02227 4.8094V3.96565ZM11.7285 16.2563H6.27227C5.79414 16.2563 5.40039 15.8906 5.37227 15.3844L4.95039 6.2719H13.0785L12.6566 15.3844C12.6004 15.8625 12.2066 16.2563 11.7285 16.2563Z"
                                                                fill="" />
                                                            <path d="M9.00039 9.11255C8.66289 9.11255 8.35352 9.3938 8.35352 9.75942V13.3313C8.35352 13.6688 8.63477 13.9782 9.00039 13.9782C9.33789 13.9782 9.64727 13.6969 9.64727 13.3313V9.75942C9.64727 9.3938 9.33789 9.11255 9.00039 9.11255Z"
                                                                fill="" />
                                                            <path
                                                                d="M11.2502 9.67504C10.8846 9.64692 10.6033 9.90004 10.5752 10.2657L10.4064 12.7407C10.3783 13.0782 10.6314 13.3875 10.9971 13.4157C11.0252 13.4157 11.0252 13.4157 11.0533 13.4157C11.3908 13.4157 11.6721 13.1625 11.6721 12.825L11.8408 10.35C11.8408 9.98442 11.5877 9.70317 11.2502 9.67504Z"
                                                                fill="" />
                                                            <path
                                                                d="M6.72245 9.67504C6.38495 9.70317 6.1037 10.0125 6.13182 10.35L6.3287 12.825C6.35683 13.1625 6.63808 13.4157 6.94745 13.4157C6.97558 13.4157 6.97558 13.4157 7.0037 13.4157C7.3412 13.3875 7.62245 13.0782 7.59433 12.7407L7.39745 10.2657C7.39745 9.90004 7.08808 9.64692 6.72245 9.67504Z"
                                                                fill="" />
                                                        </svg>
                                                    </button>
                                                </form>
                                            @endif
                                        </div>
                                    </td>
                                @endif
                            </tr>
                        @endforeach
                    </tbody>
                </table>
            </div>
        </div>
        <!-- ====== Table Three End -->
    </div>
    <!-- ====== Table Section End -->
@endsection
