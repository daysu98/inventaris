    ...
    <div class="flex flex-col gap-10">
        ...
        <div class="rounded-sm border border-stroke bg-white px-5 pb-2.5 pt-6 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
            <div class="flex flex-wrap gap-5 xl:gap-7.5 mb-4">
                <a href="{{ route('suppliers.create') }}" class="inline-flex items-center justify-center gap-2.5 rounded-md bg-primary px-5 py-2 text-center font-medium text-white hover:bg-opacity-90">
                    <span>
                    ...
                    <span class="hidden sm:flex">Tambah Supplier</span>
                </a>
            </div>
                ...
                <table class="w-full table-auto">
                    <thead>
                        <tr class="bg-gray-2 text-left dark:bg-meta-4">
                            <th class="min-w-[100px] px-4 py-4 font-medium text-black dark:text-white xl:pl-11">
                                No
                            </th>
                        ...
                        @foreach ($suppliers as $supplier)
                            <tr>
                                ...
                                <td class="border-b border-[#eee] px-4 fpy-5 dark:border-strokedark">
                                    <p class="text-black dark:text-white">{{ $supplier->phone }}</p>
                                </td>
                                <td class="border-b border-[#eee] px-4 fpy-5 dark:border-strokedark">
                                    <textarea rows="6" readonly
                                        class="min-w-[250px] rounded-lg border-[1.5px] border-stroke bg-transparent px-5 py-3 font-normal text-black outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:text-white dark:focus:border-primary">{{ $supplier->address }}</textarea>
                                </td>
                                <td class="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                                    <div class="flex items-center space-x-3.5">
                                        <a href="{{ route('suppliers.edit', ['supplier' => $supplier->id]) }}" class="hover:text-primary">
                                        ...
                                        <form action="{{ route('suppliers.destroy', ['supplier' => $supplier->id]) }}" method="POST" style="height: 20px;">
                                            @csrf
                                            @method('DELETE')
